...
export default class PoolSelector {
  ...
  private async getCheckedPool(pool: Pool, poolInfo: any): Promise<Pool> {
    ...
    const meetsStakingRequirement = await this.getRootMeetsStakeRequirement(
      poolInfo.roles.root
    );
    ...
    if (this.checkValidators) {
      pool.pass = await this.getValidatorsMeetCriteriaByPoolId(
        pool.poolStashAccountId
      );
    } else {
      pool.pass = true;
    }
  ...
  private async getValidatorsMeetCriteriaByPoolId(
    ...
    const duplicatesOrNotChecked = await this.getHasDuplicateValidators(
      targets
    );
  ...
  private async getValidatorsMeetCriteria(targets: string[]): Promise<boolean> {
    for (let t of targets) {
      const meetsCriteria =
        await this.validatorSelector.getMeetsCriteriaByAccountId(t);
      if (!meetsCriteria) {
        return false;
      }
    }
  ...
  async getPoolsMeetingCriteria(): Promise<Pool[]> {
    ...
    const matchingPools = [];
    const numberOfPools =
      await this.api.query.nominationPools.counterForRewardPools();
    const randomisedOrder = PoolSelector.randomiseOrder(
      numberOfPools.toNumber()
    );
